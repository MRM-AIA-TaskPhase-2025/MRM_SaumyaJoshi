...
while True:
    ...
    # Threshold the Laplacian edges to get binary edges
    _, binary_laplacian_edges = cv2.threshold(laplacian_edges, 50, 255, cv2.THRESH_BINARY)

    # Create an empty mask for the background
    background_mask = np.zeros_like(binary_laplacian_edges)

    # Copy edges to the background mask
    background_mask[binary_laplacian_edges > 0] = 255
